...
5. Confirm authorizes, marks the batch `PROCESSING`, and returns immediately. It does not start a thread. GET status never mutates.  
6. Render worker `wms-v2-import-worker` (`python -m app.workers.import_worker`) is primary: it heartbeats and claims `PROCESSING` batches. If the heartbeat is missing/stale, the UI automatically POSTs CSRF-protected `/imports/<id>/recover` for bounded chunks. Advisory locks prevent overlap. `flask recover-imports` is emergency-only. `POST /inventory/imports/<id>/advance` remains admin-only.  
...
4. Rows group into fulfillment orders by Client + Division + Warehouse + raw Order Number + Customer + Customer Address (customer/address normalized for identity). Header consistency is required only inside that group. Different customers under one raw Order Number become separate WMS orders. Blank phone/carrier/service are valid; nonblank conflicts inside one group reject the file.  
5. Destination sequence is deterministic: for each Client + raw Order Number, sort groups by warehouse_code ASC, normalized customer ASC, normalized address ASC and assign 01, 02, 03…. `wms_order_id = {client_code}-{client_order_number}-{seq:02d}`.  
