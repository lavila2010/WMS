"""Durable import worker for inventory and order batches.
...
claims one batch at a time with the same advisory lock used by the matching
processor. A restart resumes from ``units_created`` / ``orders_created``.
