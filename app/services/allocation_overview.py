...
def _overview_sql(candidate_sql: str) -> str:
    ...
    open_status = PickTicketStatus.OPEN
    return f"""
        ...
        SELECT o.id AS order_id,
               ...
               GREATEST(
                   COALESCE(l.ordered, 0) - COALESCE(l.shipped, 0) - COALESCE(lv.currently, 0),
                   0
               ) AS remaining,
...
def _row_flags(row) -> dict:
    ...
    return {
        ...
        "eligible": remaining > 0 and row.status not in {OrderStatus.CLOSED, OrderStatus.CANCELLED},
...
def allocation_overview_page(
    *,
    ...
    for row in sliced:
        ...
        payload["unticketed"] = payload["unticketed"]
        rows.append(payload)
    return {
        "rows": rows,
        "page": page,
...
def bulk_upc_rows(order_ids: list[int]) -> dict[int, list[dict]]:
    ...
    line_sql = text(
        ...
        GROUP BY ol.id, ol.order_id, ol.upc, ol.sku, ol.description, ol.qty_ordered, ol.qty_shipped
    ...
    for row in db.session.execute(line_sql):
        ...
        remaining = max(int(row.qty_ordered) - int(row.qty_shipped or 0) - live, 0)
