...
def _attach_allocation_actions(rows: list[dict]) -> list[dict]:
    """Keep newest action-queue labels while the list itself stays bulk-SQL."""
    for row in rows:
        ticket_ok = bool(row.get("pick_ticket_eligible") or row.get("pick_ticket_ready"))
        order = row["order"]
        row["pick_ticket_ready"] = ticket_ok
        if ticket_ok:
            row["action_label"] = "Ready for Pick Ticket"
            if current_user.has_permission("PICK_TICKET_VIEW"):
                row["action_href"] = url_for("orders.pick_tickets", **_pick_tickets_filter_args(order))
            else:
                row["action_href"] = url_for("allocation.detail", order_id=order.id)
        elif row["currently_allocated"] > 0 and row["remaining"] > 0 and row.get("partial_approval") != "APPROVED":
            row["action_label"] = "Approve Partial"
            row["action_href"] = url_for("allocation.approve_partial", order_id=order.id)
        else:
            row["action_label"] = "Allocate"
            row["action_href"] = url_for("allocation.detail", order_id=order.id)
    return rows
...
def index():
    ...
    rows = _attach_allocation_actions(page_data["rows"])
