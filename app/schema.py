...
def ensure_v2_schema() -> None:
    ...
    with bind.begin() as conn:
        ...
        for idx in (
            ...
            "CREATE INDEX IF NOT EXISTS ix_orders_client_status_created ON orders (client_id, status, created_at)",
            "CREATE INDEX IF NOT EXISTS ix_alloc_order_status ON allocations (order_id, status)",
            "CREATE INDEX IF NOT EXISTS ix_alloc_line_status ON allocations (order_line_id, status)",
            "CREATE INDEX IF NOT EXISTS ix_alloc_order_wave_ticket ON allocations (order_id, wave_number, pick_ticket_id)",
            "CREATE INDEX IF NOT EXISTS ix_units_allocation ON inventory_units (allocation_id)",
            "CREATE INDEX IF NOT EXISTS ix_units_alloc_order_status ON inventory_units (allocated_order_id, status)",
            "CREATE INDEX IF NOT EXISTS ix_units_cwus_loc_id ON inventory_units (client_id, warehouse_id, upc, status, location, id)",
            "CREATE INDEX IF NOT EXISTS ix_pick_tickets_order_status ON pick_tickets (order_id, status)",
