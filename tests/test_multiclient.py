...
def test_wrong_order_type_barcode_rejected(db):
    order = _validated_order(order_number="SO-T", client="ACME", warehouse="WH1", order_type="B2C", lines=[("SKU-A", 1)])
    make_unit("WT-1", "SKU-A", client="ACME", warehouse="WH1", order_type="B2B")
    with pytest.raises(BarcodeError) as exc:
        allocate_barcode(order, "WT-1")
    assert exc.value.exc_type == ExceptionType.WRONG_ORDER_TYPE
